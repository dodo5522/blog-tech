...
作業を開始する前に、GitHub の open Issue を確認し、対象 Issue の残件と優先順位を把握してください。
`.steering/` 配下の文書は過去の作業記録としてのみ参照し、新しい残件の追加や既存残件の更新には使用しないでください。
作業中に新たな残件が見つかった場合は GitHub Issue を作成または更新し、対応が完了した Issue は検証結果を残して close してください。
...
- 「ブログ化」「記事化」などの記事作成依頼では、記事パッケージの作成と検証に加え、Sanity Draft の作成まで行うこと。
- Sanity Draft は公開せず、publish は明示的に依頼された場合だけ行うこと。
- 素材が長く、ひとつの記事では主題が散る場合は、テーマの自然な境目で2本または3本に分けること。
- ChatGPT の共有 URL など、素材を取得するための非公開 URL は、Issue、Pull Request、コミットメッセージ、記事本文へ記載しないこと。
- 共有 URL や抽出した会話は Git 管理外の一時領域だけで扱い、Draft 作成後に削除すること。
