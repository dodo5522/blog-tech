...
import { PostFieldsWithoutBody } from 'tina/types';
import { Category } from 'tina/collections/post/types';
    ...
    props: {
      posts: postsList.data.postConnection.edges?.map(p => {
        return {
          ...
          tags: [],
          author: '',
          categories: (p?.node?.categories ?? []) as Category[],
          id: p?.node?.id ?? '',
        };
      }) ?? []
    },
...
interface PostCollectionFieldsWithFileName extends PostFieldsWithoutBody {
  filename: string;
}

interface HomePageProps {
  posts: PostCollectionFieldsWithFileName[];
}
